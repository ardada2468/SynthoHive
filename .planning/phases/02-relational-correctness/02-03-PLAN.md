...
Output: linkage.py rewritten to use empirical (numpy.random.choice) or NegBinom (scipy.stats.nbinom) based on the method parameter. GaussianMixture import removed. Fallback from NegBinom to empirical when distribution is not overdispersed.
</objective>
...
<task type="auto">
  ...
  <files>syntho_hive/relational/linkage.py</files>
  <action>
