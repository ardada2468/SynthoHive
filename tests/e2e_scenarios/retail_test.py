...
def generate_ground_truth():
    ...
    for i in range(1, 6):
        regions.append(
            {"region_id": i, "region_code": fake.state_abbr(), "country": "USA"}
        )
...
def sanitize_data():
    ...
    privacy_conf = PrivacyConfig(
        rules=[
            PiiRule(name="email", patterns=[r"[^@]+@[^@]+\.[^@]+"], action="fake"),
            PiiRule(
                name="phone", patterns=[r".*"], action="fake"
            ),  # Simple catch-all for phone
            PiiRule(
                name="address", patterns=[r"\d+ .+,.*"], action="fake"
            ),  # Heuristic address
...
def train_and_generate():
    ...
    spark = (
        SparkSession.builder.appName("SynthoHiveRetailTest")
        .master("local[*]")
    ...
    meta.add_table(
        ...
        constraints={"age": Constraint(dtype="int", min=18, max=90)},
        parent_context_cols=["region_code"],
    )  # Condition on region code

    meta.add_table(
        "orders",
        pk="order_id",
        fk={"user_id": "users.user_id"},
        constraints={"total": Constraint(dtype="float", min=0.0, max=10000.0)},
        parent_context_cols=["age"],
    )  # Spending might depend on age

    meta.add_table(
        "order_items",
        pk="item_id",
        fk={"order_id": "orders.order_id", "product_id": "products.product_id"},
        constraints={"quantity": Constraint(dtype="int", min=1, max=3000)},
        parent_context_cols=["total"],
    )  # Quantity/Products might relate to total is better -> order_total in csv is "total"
    ...
    meta.add_table(
        ...
        constraints={"delivery_days": Constraint(dtype="int", min=1, max=10)},
...
def validate_results():
    ...
    report.generate(real_data=real_data, synth_data=synth_data, output_path=REPORT_PATH)
