...
def test_ctgan_full_cycle():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        save_path = os.path.join(tmp_dir, "test_ctgan_model.pth")
        model.save(save_path)
        assert os.path.isdir(save_path), "save() should produce a directory checkpoint"
...
def test_ctgan_conditional():
    ...
    # 1. Setup Data
    # Child: Orders
    orders = pd.DataFrame({"amount": np.random.normal(100, 20, 100)})

    # Context: User Income (Parent)
    context = pd.DataFrame({"user_income": np.random.normal(50000, 10000, 100)})
    ...
    new_context = pd.DataFrame({"user_income": [60000, 40000]})
...
def test_multi_table_generation_print(capsys):
    ...
    context_data = merged[
        ["income", "region"]
    ]  # Context used to condition the generation
